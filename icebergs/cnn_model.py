...
if new_model:
    ...
    model = load_model('icebergs/models/denoised2.h5')
...
valgen = ImageDataGenerator(featurewise_center=False,featurewise_std_normalization=False)
valgen.fit(X_test)
...
if new_model:
    ...
    history = model.fit_generator(datagen.flow(X_train,Y_train,batch_size=batch_size), steps_per_epoch = int(len(X_train)/batch_size),epochs=2000,validation_data=valgen.flow(X_test,Y_test),verbose=1,initial_epoch=1000,callbacks=[checkpointer])

model.save('icebergs/models/new_model.h5')
